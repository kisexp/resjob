...
func (cc *perIPConnCounter) Unregister(ip uint32) {
	...
	if cc.m == nil {
		cc.lock.Unlock()
		panic("perIPConnCounter.Register() wasn't called")
	}
	n := cc.m[ip] - 1
	if n < 0 {
		cc.lock.Unlock()
		panic(fmt.Sprintf("negative per-ip counter=%d for ip=%d", n, ip))
	}
	cc.m[ip] = n
	cc.lock.Unlock()
}
...
func (c *perIPConn) Close() (err error) {
	err = c.Conn.Close()
...
func getUinit32IP(c net.Conn) uint32 {
